import datetime, pytz, re, aiofiles, subprocess, db, os, base64, io, server
...
async def without_login(bot, user_id, m, all_urls, start_time, bname, batch_id, app_name, price=None, start_date=None, imageUrl=None):
    ...
    await bot.send_document(chat_id=Config.TXT_LOG, document=file_path, caption=caption, thumb=thumb)
...
async def login(bot, user_id, m, all_urls, start_time, bname, batch_id, app_name, price=None, start_date=None, imageUrl=None):
    ...
    async with aiofiles.open(file_path, 'w', encoding='utf-8') as f:
        await f.writelines([url + '\n' for url in all_urls])
    copy = await m.reply_document(document=file_path,thumb=thumb,caption=caption)
    await copy.copy(chat_id=Config.TXT_LOG)
    await db.db_instance.save_backup_file(user_id, caption, file_path)
    os.remove(file_path)

async def login_free(bot, user_id, m, all_urls, start_time, bname, batch_id, app_name, price=None, start_date=None, imageUrl=None):
    bname = await server.sanitize_bname(bname)
    file_path = f"{bname}.txt"
    file_name_enc = f"{bname}_enc.txt"
    local_time = datetime.datetime.now(pytz.timezone('Asia/Kolkata'))
    ...
    async with aiofiles.open(file_path, 'w', encoding='utf-8') as f:
        await f.writelines([url + '\n' for url in all_urls])
    await file_name_encr(all_urls, file_name_enc)
    file = file_path if await db.db_instance.get_premium_user(user_id) else file_name_enc
